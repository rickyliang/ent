...
LatLngBounds.prototype.init_ = function(sw, ne) {
    // Make sure our southwestern point and our northwestern point
    // do not have the exact same latitude and longitude.
    if (sw.lat() == ne.lat() && sw.lng() == ne.lng()) {
        sw = new LatLng(sw.lat() - OFFSET, sw.lng() - OFFSET);
        ne = new LatLng(ne.lat() + OFFSET, ne.lng() + OFFSET)
    }
    this.sw = sw;
    this.ne = ne;
