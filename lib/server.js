...
function getDistanceMatrixRequestOptions(origins, destinations) {
    ...
    var options = {
        ...
        path: '/maps/api/distancematrix/json?key=' + API_KEY + '&origins=' + origins + '&destination=' + destinations
    }
    return options;
}
        ...
        response.on('end', function () {
            ...
            Promise.all(yelpPromises).then(function (responses) {
                ...
                for (i = 0; i < responses.length; i++) {
                    businesses = businesses.concat(responses[i].businesses);
                }
                ...
                var distanceMatrixPromises = [];
                console.log('Calling DistanceMatrix');
                var originToBusiness = new Promise(function (resolve, reject) {
                    console.log('origin: ' + origin);
                    console.log(businessLocations);
                    https.get(getDistanceMatrixRequestOptions([origin], businessLocations), function (response) {
                        response.setEncoding('utf-8');
                        var result = '';
                        response.on('data', function (data) {
                            result += data;
                ...
                var businessToDestination = new Promise(function (resolve, reject) {
                    https.get(getDistanceMatrixRequestOptions(businessLocations, [destination]), function (response) {
                        response.setEncoding('utf-8');
                        var result = '';
                        response.on('data', function (data) {
                            result += data;
                ...
                distanceMatrixPromises.push(originToBusiness);
                distanceMatrixPromises.push(businessToDestination);
                console.log('Pushing distance matrix promises');

                Promise.all(distanceMatrixPromises).then(function (responses) {
                    console.log('All distance matrix promises resolved');
