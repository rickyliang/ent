...
app.get('/api/directions', function (req, res) {
    ...
    httpsGet(options).then(function (result) {
        ...
        // Get total route duration text and seconds
        totalDurationText = result.routes[0].legs[0].duration.text;
        totalDurationValue = result.routes[0].legs[0].duration.value;
        ...
        // TODO: For each business, calculate the total duration
        // from origin -> business -> destination
...
function getDistanceMatrixRequestOptions(origins, destinations) {
    ...
    var durationInTrafficOption = '&mode=driving&departure_time=now';
    var unitsOption = '&units=imperial';
    var options = {
        hostname: 'maps.googleapis.com',
        path: '/maps/api/distancematrix/json?key=' + GOOGLE_API_KEY + '&origins=' + origins + \
              '&destinations=' + destinations + durationInTrafficOption + unitsOption
    }
    return options;
}
